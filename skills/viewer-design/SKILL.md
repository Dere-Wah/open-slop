...
| `lib/types.ts` | The TypeScript side of `PROTOCOL.md`: `ShowState` (the `show.state` cursor) and `Rundown` (room metadata). The two move together; a field added here is added there. |
| `lib/partners.ts` | The Infinite: name, URL, favicon, the `infinite-` identity prefix, the allowed origins for its token endpoint. `sourceOfIdentity` is the one place a chat line's origin is decided. |
| `app/api/infinite/token/route.ts` | A CORS-fenced token for theinfinite.tv: subscribe-only, 15 minutes, `?chat=1` for the data grant. The protocol it serves is `INTEGRATION.md`. |
| `components/Overlay.tsx` | The "On air" chip (a red dot and the two words), a viewer count beside it (an eye and a number, `1.3k` past a thousand), the "Running on Reactor" chip top-right, the stall ribbon, and (from `sm` up) a green "Edit this scene" button bottom-left opening GitHub's editor on the playing lines, drawn over the picture while live. Below `sm` the frame is too small; the now-playing bar's "Edit scene" button stands in. |
...
| `components/Chat.tsx` | The room chat, first in the sidebar. Fixed height from its `className`, scrolls inside, follows new messages only when the reader is at the bottom; a header toggle collapses it to one row (remembered in localStorage). No name field: the first send opens a small dialog asking for one, which then persists; "change" under the box reopens it. A link to one of this repository's pull requests renders as a chip (`#41` with the pull-request icon) through `pullRefOf`; every other URL stays plain text. A line typed in The Infinite's theatre (entry `source: "infinite"`, set by ShowApp off the identity) gets their favicon after the name, a button that opens a small "Spectate this from a movie theatre!" dialog linking there. |
