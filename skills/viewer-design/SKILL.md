...
| `components/RepoHeader.tsx` | Owner / repo, `Public`, branch @ sha, the three buttons, the underline nav. "Write the next scene" is the page's one call to action: `gh-btn-lg` makes it taller than its neighbours, and it opens the story branch's `CONTRIBUTING.md` (`contributingUrl`), the three-step guide — never the README. A connection pill sits at the nav's right end only while the page is not connected; there is no global bar above it. |
