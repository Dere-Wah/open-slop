...
| `PROTOCOL.md` | The wire: the two tracks, the `show.state` cursor and its statuses, the rundown in room metadata, and the `show.chat` packet, field by field. Anyone in the room can build a presentation from it. Changes to `broadcast.py`, `publisher.py`, or `sendChat` update it in the same change. |
| `INTEGRATION.md` | The contract for a partner site showing the screening: the `/api/infinite/token` endpoint, the room's tracks and topics, and the shared chat protocol. |
